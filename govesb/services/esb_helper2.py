...
from govesb.models.enums import (
...
class ESBHelper2:
    ...
    def esb_request_with_token(client_id, client_secret, api_code, esb_body, format, key, token_url, request_url):
        ...
        return ESBHelper.esb_request(api_code, token_response.access_token, esb_body, format, key, request_url)
    ...
    def esb_request(api_code, access_token, esb_body, format, key, url):
        ...
        if format == DataFormatEnum.xml:
            ...
            payload = ESBHelper.create_request(api_code, access_token, esb_body_xml, format, key, ModeOfConnection.PULL)
        else:
            payload = ESBHelper.create_request(api_code, access_token, esb_body_json, format, key, ModeOfConnection.PULL)
    ...
    def esb_push_request(push_code, access_token, esb_body, format, key, url):
        ...
        payload = ESBHelper.create_request(push_code, access_token, esb_body, format, key, ModeOfConnection.PUSH)
        response = requests.post(url, data=payload.encode("utf-8"), headers=headers)
        return response.text
    ...
    def verify_signed_payload(received_data, format_enum, public_key):
        try:
            if format_enum == DataFormatEnum.json:
                ...
                data = ESBHelper.extract_real_data(received_data)
    ...
    @staticmethod
    def verify_and_extract_data(received_data, format_enum, public_key):
        result = ResponseData()
        try:
            if format_enum == DataFormatEnum.json:
